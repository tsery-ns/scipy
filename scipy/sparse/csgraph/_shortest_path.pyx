...
""""
cdef class DijkstraNodeData:
    FibonacciState state
    unsigned int source
    unsigned int index

ctypedef fused NodeData:
    DijkstraNodeData
"""
...
def yen(
    ...
    if unweighted:
        ...
        if np.any(csr_data < 0):
            # Use jonsons algorithm to handle negative weights
...
@cython.boundscheck(False)
cdef int _yen_is_path_in_candidates(
    int[:, :] candidate_predecessors,
    int[:] orig_path, int[:] spur_path,
    int spur_node, int sink
):
    """
    Return 1 if the path cosisted by orig_path + spur_path
    is in candidate_predecessors, 0 otherwise
    """
    cdef int i
    cdef int node
